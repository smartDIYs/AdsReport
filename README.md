...
- **パスワード認証**: アクセス時にパスワード入力を要求

## デプロイ

Streamlit Community Cloud にデプロイ済みです。GitHubへのpushで自動的に反映されます。

認証情報は Streamlit Cloud の **Settings → Secrets** でTOML形式で管理しています。
...
| 項目 | 取得元 |
|---|---|
| DEVELOPER_TOKEN | Google Ads MCC管理画面 → ツールと設定 → APIセンター |
...
| 項目 | 取得元 |
|---|---|
| CLIENT_ID / CLIENT_SECRET | Yahoo!広告 API 管理画面（connect-business.yahoo.co.jp）でアプリ登録 |
| ACCOUNT_ID | Yahoo!広告の広告アカウントID |
| MCC_ACCOUNT_ID | Yahoo!広告のMCCアカウントID（x-z-base-account-id ヘッダに使用） |
...
| 項目 | 取得元 |
|---|---|
| CLIENT_ID | Azure AD → アプリの登録（パブリッククライアント/個人用アカウント） |
| DEVELOPER_TOKEN | Microsoft Advertising → ツール → 開発者トークン |
| ACCOUNT_ID / CUSTOMER_ID | Microsoft Advertising 管理画面のURL（aid / cid パラメータ） |
...
`config/category_mapping.yaml` の `product_mapping` に商品名→カテゴリを追加するだけで対応できます（コード変更不要）。
